...
def multibit_negative(A):
    ...
    negA = []
    for a in A:
        negA.append(NOT(a)) # First, flip all ones to zeros, and zeros to ones
        
    B = [0]*len(A) # Then create an array the length of A
    B[0] = 1 # And set the binary value to 1
    
    negA = multibit_adder(negA,B) # Finally, add one to the inversed array of A
    return negA
...
def multibit_subtractor(A, B):
    ...
    B = multibit_negative(B) # make B negative
    S = multibit_adder(A,B) # add A and -B
