...
class CoupledOscillators:
    ...
    def __init__(self, X0=[-0.5, 0, 0.5], m=1.0, k=1.0):
        ...
        K = np.zeros(shape=[len(X0), len(X0)])

        for ii in range(len(X0)):
            for jj in range(len(X0)):
                if ii == jj:
                    K[ii][jj] = 2 # set to 2 where i=j
                elif abs(ii - abs(jj)) == 1:
                    K[ii][jj] = -1 # set to -1 within 1 position from diagonal 
        

        # Normal modes become the eigenvectors and values of matrix K:
        # Omega is just the square root of the eigenvalues      
        self.Omega = np.sqrt(np.linalg.eig(K*k/m)[0])
        #Store eigenvectors as V
        self.V = np.linalg.eig(K*k/m)[1]

        self.M0 = self.V.T @ X0 # matrix multiply the eigenvectors and our initial displacements for normal mode decomposition
